...
- Put validation, filtering, enrichment, aggregation, and model/feature computation as processing stages.
- Show data stores near the stage that writes or owns them.
- Label arrows with data type, topic/table name, or cadence only if supplied by the source.

Styling:
- Use distinct but restrained colors for source, processing, storage, and serving nodes.
